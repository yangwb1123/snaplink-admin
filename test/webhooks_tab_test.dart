...
const _webhookEndpoints = [
  ...
  SnaplinkAdminEndpoint(
    method: 'DELETE',
    path: '/api/v1/admin/webhooks/subscriptions/{id}',
    feature: 'core',
  ),
  SnaplinkAdminEndpoint(
    method: 'GET',
    path: '/api/v1/admin/webhooks/deadletters',
    feature: 'core',
  ),
  SnaplinkAdminEndpoint(
    method: 'POST',
    path: '/api/v1/admin/webhooks/deadletters/{id}/replay',
    feature: 'core',
  ),
];
...
void main() {
  group('WebhooksTab capabilities', () {
    test('detects webhook availability from endpoint inventory', () {
      ...
      expect(
        caps.hasAnyPathPrefix('/api/v1/admin/webhooks/subscriptions'),
        isTrue,
      );
      expect(caps.has('POST', '/api/v1/admin/webhooks/subscriptions'), isTrue);
      expect(
        caps.has('DELETE', '/api/v1/admin/webhooks/subscriptions/{id}'),
        isTrue,
      );
    });
    ...
    test('detects dead letter availability', () {
      final caps = SnaplinkAdminCapabilities(_webhookEndpoints);
      expect(
        caps.hasAnyPathPrefix('/api/v1/admin/webhooks/deadletters'),
        isTrue,
      );
    });
  ...
  group('SnaplinkAdminOperationCatalog includes webhook routes', () {
    test('webhook subscription routes are documented', () {
      ...
      expect(
        documented.any((e) => e.path.contains('/webhooks/subscriptions')),
        isTrue,
      );
    });
    ...
    test('webhook dead letter routes are documented', () {
      final documented = SnaplinkAdminOperationCatalog.endpoints;
      expect(
        documented.any((e) => e.path.contains('/webhooks/deadletters')),
        isTrue,
      );
    });
  });
