...
void main() {
  ...
  group('_handleSuccess census (REQ-1/REQ-4)', () {
    ...
    test('adjacency rule: every `if (outcome.ok) {` is immediately followed by '
    ...
    test('no auth.login.success emission string in the module (REQ-4 #5)', () {
      ...
      for (final entity in Directory(moduleDir).listSync()) {
        ...
        if (File(
          entity.path,
        ).readAsStringSync().contains('auth.login.success')) {
  ...
  group('client_id literal census (single-source rule, §6.4 gate c)', () {
    ...
    test('literal census derives from constant existence + standing 43-test '
      ...
      final constantExists = File(
        ssoClientPath,
      ).readAsStringSync().contains(constantDecl);
      final actual = <String, List<int>>{};
      ...
      expect(
        censusLines.indexWhere(
          (l) => RegExp(
            r"^\s*for \(final entity in Directory\('lib'\)",
          ).hasMatch(l),
        ),
        isNot(-1),
      ...
      if (constantExists) {
        ...
        expect(
          ...
          isEmpty,
          reason:
              'firstPartyClientId exists in $ssoClientPath — the '
        ...
        expect(
          ...
          1,
          reason:
              'exactly one lib/ site may carry the literal in '
              'constant mode — found $libOffenders',
        );
        expect(
          libOffenders.keys.single,
          ssoClientPath,
          reason:
              'the single lib/ site must be the declaration file '
        ...
        expect(
          File(
            ssoClientPath,
          ).readAsStringSync().split('\n')[declLine - 1].contains(constantDecl),
          isTrue,
          reason:
              'the single lib/ hit must be the declaration line '
        ...
        expect(
          ...
          pinnedSites,
          reason:
              'constant absent — literal census is pinned; the '
      ...
      expect(
        ...
        10,
        reason:
            'census file must hold 10 tests (6 census/literal-census '
      ...
      expect(
        ...
        3,
        reason:
            'exactly-once harness file must hold 3 tests — found '
            '$ssoLoginCount',
      );
      ...
      expect(
        ...
        10,
        reason:
            'entry_ux_test must hold 10 tests (redirect-leg group '
      ...
      for (final f in [
        ...
        expect(
          silenceRe.hasMatch(File(f).readAsStringSync()),
          isFalse,
  ...
  group('single-emission topology (REQ-5, revision-3 pins)', () {
    ...
    List<(String, String, int, int, List<String>)> sliceMethods(String path) {
      ...
      while (i < lines.length) {
        ...
        if (sig.hasMatch(line) &&
          ...
          if (j < lines.length) {
            result.add((path, name, i + 1, j + 1, lines.sublist(i, j + 1)));
            i = j + 1;
            continue;
          }
        }
    ...
    test(
      ...
      () {
        ...
        expect(
          ...
          6,
          reason:
              'identified set must be exactly 6 (Gap A) — a dispatch '
        ...
        pinned.forEach((file, lineNumbers) {
          ...
          for (final ln in lineNumbers) {
            ...
            expect(
              ...
              1,
              reason:
                  'Gap A containment: $file:$ln must lie inside '
        ...
        for (final m in ids) {
          expect(
            ...
            1,
            reason:
                '${m.$1}:${m.$3} must contain exactly one '
                '_handleSuccess(outcome); call site (exact-form '
                'criterion)',
          );
          expect(
            m.$5.where((l) => l.contains('_handleSuccess(')).length,
            1,
            reason:
                '${m.$1}:${m.$3} must contain exactly one '
        ...
        final hs = all.firstWhere(
          (m) => m.$1 == authorizationFlow && m.$3 == 8,
        );
        final region = hs.$5.sublist(1, hs.$5.length - 1);
        final stmtRe = RegExp(
          r'^\s*(_update\(|_authorizationDeliveryBlocked\()',
        );
        final starts = <int>[];
        ...
        expect(
          ...
          8,
          reason:
              'G2 presence anchor: exactly 8 delivery tails inside '
        ...
        for (final s in starts) {
          ...
          expect(
            ...
            isTrue,
            reason:
                'G2: tail at authorization_flow:${hs.$3 + s + 1} has '
                'no terminating ;',
          );
          final functionFinal =
              t == region.length - 1 ||
          ...
          var nxt = t + 1;
          while (nxt < region.length && region[nxt].trim().isEmpty) {
            nxt++;
          }
          expect(
      ...
      () {
        ...
        for (final anchor in [
          ...
          expect(
            ...
            isNot(-1),
            reason:
                'Gap B: anchor `$anchor` must exist in _submitLogin '
          ...
          expect(
            ...
            'return;',
            reason:
                'Gap B: next non-empty line after `$anchor` '
    ...
    test('one wire mutation per dispatch method (== 1) + choke point '
        'request-free (G1)', () {
      final all = allSlices();
      final ids = [
        for (final m in all)
      ...
      for (final m in ids) {
        ...
        expect(
          ...
          1,
          reason:
              '${m.$1}:${m.$3} must contain exactly one wire '
              'mutation (== 1, not <= 1) — a second POST per method '
              'fails here',
        );
      }
      final hs = all.firstWhere((m) => m.$1 == authorizationFlow && m.$3 == 8);
      expect(
        hs.$5.isNotEmpty,
        isTrue,
      ...
      expect(
        hs.$5.where((l) => l.contains('_api.')).length,
        0,
        reason:
            'G1: _handleSuccess must stay request-free — a wire '
            'added inside the choke point creates the double-request '
            'vector AC-2 targets',
      );
    });

    test('Gap C: renewal call site pinned at provider_flow:36 inside '
      ...
      expect(
        ...
        1,
        reason:
            'Gap C: exactly one _submitSilentRenewal( reference '
            'outside its declaration (module-wide over $moduleDir)',
      );
      expect(
        refs.containsKey(providerFlow),
        isTrue,
      ...
      expect(
        ...
        36,
        reason:
            'Gap C positional pin: the reference must be at '
            'provider_flow:36 (a relocation into an interactive handler '
            'keeps the count at 1 but fails here)',
      );
      final cfr = allSlices().firstWhere(
        (m) => m.$1 == providerFlow && m.$2 == '_checkFederatedReturn',
      );
      expect(
        cfr.$3 <= 36 && 36 <= cfr.$4,
        isTrue,
        reason:
            'Gap C: provider_flow:36 must lie inside the '
            '_checkFederatedReturn slice (${cfr.$3}-${cfr.$4})',
      );
      final viewLines = linesOf('$moduleDir/oidc_login_view_flow.dart');
      expect(
        viewLines.where((l) => l.contains('onSubmit: _submitLogin,')).length,
        1,
      ...
      for (final entity in Directory(moduleDir).listSync()) {
        ...
        for (var i = 0; i < lines.length; i++) {
          if (lines[i].contains('onSubmit:') &&
            ...
            fail(
              'onSubmit must never reference _submitSilentRenewal '
              '(${entity.path}:${i + 1})',
            );
          }
        }
