...
class _LoginHarness {
  _LoginHarness() {
    client = MockClient((request) async {
      ...
      if (decoded is! Map<String, dynamic> || decoded['credential'] is! Map) {
        fail(
...
void _expectCumulativeTwo(List<_RecordedLogin> recorded) {
  expect(
    ...
    2,
    reason:
        'two login() calls must yield exactly two requests (cumulative '
...
void main() {
  group('SSOAdminClient.login exactly-once emission (REQ-2 / AC-1)', () {
    test(
      'login() issues exactly one credential-bearing POST /auth/login',
      () async {
        ...
        expect(
          ...
          isTrue,
          reason:
              'the access_token from the single response installs the '
              'session',
        );
      },
    );

    test('the single request carries the credential body with the '
        'first-party client id (F4/F6) — scalar asserts, credential value '
        'never enters a diff', () async {
      final harness = _LoginHarness();
      final client = SSOAdminClient(
        'https://sso.example.test',
        httpClient: harness.client,
      ...
      expect(
        (credentialMap['password'] as String).isNotEmpty,
        isTrue,
        reason:
            'the password must be non-empty — only its shape, '
            'never its value, may enter an assertion diff',
      );
    });

    test('the strict recording client trips on any non-login or '
        'credential-less request (no probe, F5)', () async {
      final harness = _LoginHarness();
    ...
    test('a second login() adds exactly one more request (cumulative 2, '
        'per-call bounded)', () async {
      final harness = _LoginHarness();
      final client = SSOAdminClient(
        'https://sso.example.test',
        httpClient: harness.client,
      ...
      expect(
        ...
        2,
        reason:
            'second login must issue exactly one more request — '
    ...
    test('binding: the exactly-one gate cannot pass while a second '
    ...
    test('binding: the cumulative-2 gate rejects a third request and a '
        'dropped second login', () {
      const one = _RecordedLogin(
        method: 'POST',
        path: '/auth/login',
        body: {
          'provider': 'password',
