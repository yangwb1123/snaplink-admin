...
/// 11,21).
void main() {
  test('DCR mutations leave the pre-seeded audit ring untouched (AC-1)', () async {
    ...
    final api = DeveloperApi(
      ...
      httpClient: MockClient((request) async {
        ...
        if (request.method == 'PUT' && request.url.path == '/register/client-1') {
          expect(request.headers['authorization'], 'Bearer rat-1');
          return http.Response('{}', 200);
        }
        if (request.method == 'DELETE' && request.url.path == '/register/client-1') {
          expect(request.headers['authorization'], 'Bearer rat-1');
