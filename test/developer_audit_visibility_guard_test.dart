...
/// AC-2 static guard (spec REQ-1; design §1.2): the developer module must
/// keep zero references to the debug-only audit ring — no imports, no
/// construction, no reads, no writes, no string literals.
///
/// Census pattern per `test/oidc_login_audit_visibility_guard_test.dart`
/// (`@TestOn('vm')`, `dart:io` scans of `lib/screens/developer`).
...
void main() {
  ...
  test('zero audit-ring references in the module (REQ-1 floor, AC-2)', () {
    ...
    expect(
      offenders,
      isEmpty,
      reason: 'lib/screens/developer must keep zero '
          'AuditLog' 'Service / audit_log_' 'service / sso_audit_' 'log references '
          '(localStorage ring is debug-only; DCR mutations are evidenced '
