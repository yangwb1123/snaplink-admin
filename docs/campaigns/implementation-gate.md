...
| G7（B6） | B6-1..2 | T-12 联合、console 服务端记录；**B6-1 计数断言（`flutter test … -r expanded`）**：guard+widget **+27**（device guard 1 + oidc guard 1 + entry_ux 10〔含 B6-2 组〕+ device_verify_api 4 + developer_audit_visibility_guard 1 + developer_ring_isolation 1 + developer_forge_invisibility 1 + B6-1 developer 负边界 8〔guard green 1 + 逐函数探测 3 + detail 串 pin 1 + mutation skins 3〕——guard 四文件套件基线重钉 **79**〔2026-08-08 `-r expanded` 实测：guard 37 + mutation 40 + developer guard 1 + oidc guard 1——已含 B6-1 负边界 8 与后续 b6-1a/b6-1b 同文件追加；原"56 → 64"为负边界钉写时快照，已被同文件追加超越〕）、liveness **+15**（snaplink_admin_api_test）、API **+4**（device_verify_api_test）——pin 与 liveness 套件**同门运行**，`@TestOn('vm')` 静默跳过即计数不匹配 → CI 失败；**本批次（tenant_id 注入）新增测试全部落在 pin 套件之外**（AC-1a `test/jwt_claims_test.dart` 单测 + AC-1b dashboard tenant-scoping widget 测试——均不在 pin 四文件/entry_ux/liveness/API 清单内），pin 计数不变；未来批次触碰 pin 内文件时须以 `flutter test … -r expanded` 实测重钉 |
