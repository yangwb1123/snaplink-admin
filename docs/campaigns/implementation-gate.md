...
| G7（B6） | B6-1..2 | T-12 联合、console 服务端记录；**B6-1 计数断言（`flutter test … -r expanded`）**：guard+widget **+16**（device guard 1 + oidc guard 1 + entry_ux 10〔含 B6-2 组〕+ device_verify_api 4）、liveness **+15**（snaplink_admin_api_test）、API **+4**（device_verify_api_test）——pin 与 liveness 套件**同门运行**，`@TestOn('vm')` 静默跳过即计数不匹配 → CI 失败 |
| G8（生产 cutover） | 全部批次 + 迁移骨架 P1–P3 | 各仓 CI 全绿、首个事件验证、DLQ/alert 上线（H2/H6） |

**要点**：① 批次间严格按 v2 rollout 顺序（sink → aero-id → aero-vault → IdP → aero-im → console），仅 B4-3/B4-4 与 B2/B3 可并行；② 所有验收断言均映射审查测试编号（T-1.1 至 T-12）与 drill（D1），可逐条回归；③ aero-vault B3-5 与 aero-im B5-2 的"scope 对齐/配给"依赖 B4-2（IdP scope registry），这是 F2 契约修订后的唯一剩余依赖链；④ G7(B6) 的 B6-1 计数断言必须在**同一次 CI 门禁**中同时运行 pin 套件（guard+widget +16）与 liveness 套件（+15）：`@TestOn('vm')` 在非 VM 平台静默跳过、仅 exit-code 无法拦截（M5），且 no-write pin 的对比只有在 liveness 同门证明 ring 存活时才有意义（M10）。
