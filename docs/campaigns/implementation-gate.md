...
| # | 仓库 | 文件或模块 | 改动 | 验收断言 | 依赖批次 |
|---|---|---|---|---|---|
| 1 | console | `lib/services/audit_log_service.dart`；`lib/api/audit_read_client.dart` → `SnaplinkAdminApi`；审计页 UI | 读路径接入（F-06）：审计页调 sink 读 API（tenant_id + trace_id 仅按已证明来源转发）；localStorage ring 降级为调试记录；展示服务端记录 | T-12 联合：查询触发 self-audit 行；devtools 伪造不再构成证据 | B1-5 |
| 2 | console | `lib/` 原生事件 | 边缘生成验证：login → `auth.login.success`（client_id=sso-admin-console） | sink 出现 sso-admin-console login 事件；无重复 | B4-5 |
...
| G7（B6） | B6-1..2 | T-12 联合、console 服务端记录；**B6-1 计数断言（`flutter test … -r expanded`）**：guard+widget **+27**（device guard 1 + oidc guard 1 + entry_ux 10〔含 B6-2 组〕+ device_verify_api 4 + developer_audit_visibility_guard 1 + developer_ring_isolation 1 + developer_forge_invisibility 1 + B6-1 developer 负边界 9〔guard green 1 + 逐函数探测 4〔probe 4 = camelCase letter-adjacent canary，钉死 `(?<![A-Za-z])audit`/`audit(?![A-Za-z])` 收窄类——实证两变体均零匹配〕+ detail 串 pin 1 + mutation skins 4〕——guard 四文件套件基线重钉 **83**〔2026-08-08 `-r expanded` 实测：guard 38 + mutation 43 + developer guard 1 + oidc guard 1——较 79 的 +3 = probe 4（guard）+ `_scanWith` second-consumer dispatch 分支行 + scan 6/6b 规避族 residual 行（mutation）；已含 B6-1 负边界 9 与后续 b6-1a/b6-1b 同文件追加；原"56 → 64"为负边界钉写时快照，已被同文件追加超越〕）、liveness **+19**（`snaplink_admin_api_test.dart`: 15 original liveness pins + 4 F12 query-dedup pins, re-measured 2026-08-20）、API **+4**（device_verify_api_test.dart）——pin 与 liveness 套件**同门运行**，`@TestOn('vm')` 静默跳过即计数不匹配 → CI 失败；**F12 的 4 项新增测试已纳入 liveness 记录**，其余 count-gated 文件保持不变；未来批次触碰 pin 内文件时须以 `flutter test … -r expanded` 实测重钉；**F8 计数断言已落地**（`Makefile` `guard-count-pin` 目标 + `ci.yml` 同门 step，实测 **OK +83**）——计数钉 **83**，与 G7 行同钉：删除任何 guard 测试即 +83 不匹配 → CI 失败；四文件任一变更须连同重测重钉 |
| G8（生产 cutover） | 全部批次 + 迁移骨架 P1–P3 | 各仓 CI 全绿、首个事件验证、DLQ/alert 上线（H2/H6） |

**要点**：① 批次间严格按 v2 rollout 顺序（sink → aero-id → aero-vault → IdP → aero-im → console），仅 B4-3/B4-4 与 B2/B3 可并行；② 所有验收断言均映射审查测试编号（T-1.1 至 T-12）与 drill（D1），可逐条回归；③ aero-vault B3-5 与 aero-im B5-2 的"scope 对齐/配给"依赖 B4-2（IdP scope registry），这是 F2 契约修订后的唯一剩余依赖链；④ G7(B6) 的 B6-1 计数断言必须在**同一次 CI 门禁**中同时运行 pin 套件（guard+widget +27，含 3 个已落地 developer 测试与 B6-1 developer 负边界 8 项）与 liveness 套件（+19，含 F12 四项并发去重 pin）：`@TestOn('vm')` 在非 VM 平台静默跳过、仅 exit-code 无法拦截（M5），且 no-write pin 的对比只有在 liveness 同门证明 ring 存活时才有意义（M10）。
