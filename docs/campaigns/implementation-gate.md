...
| G7（B6） | B6-1..2 | T-12 联合、console 服务端记录；**B6-1 计数断言（`flutter test … -r expanded`）**：guard+widget **+27**（device guard 1 + oidc guard 1 + entry_ux 10〔含 B6-2 组〕+ device_verify_api 4 + developer_audit_visibility_guard 1 + developer_ring_isolation 1 + developer_forge_invisibility 1 + B6-1 developer 负边界 9〔guard green 1 + 逐函数探测 4〔probe 4 = camelCase letter-adjacent canary，钉死 `(?<![A-Za-z])audit`/`audit(?![A-Za-z])` 收窄类——实证两变体均零匹配〕+ detail 串 pin 1 + mutation skins 3〕——guard 四文件套件基线重钉 **82**〔2026-08-08 `-r expanded` 实测：guard 38 + mutation 42 + developer guard 1 + oidc guard 1——较 79 的 +3 = probe 4（guard）+ `_scanWith` second-consumer dispatch 分支行 + scan 6/6b 规避族 residual 行（mutation）；已含 B6-1 负边界 9 与后续 b6-1a/b6-1b 同文件追加；原"56 → 64"为负边界钉写时快照，已被同文件追加超越〕）、liveness **+15**（snaplink_admin_api_test）、API **+4**（device_verify_api_test）——pin 与 liveness 套件**同门运行**，`@TestOn('vm')` 静默跳过即计数不匹配 → CI 失败；**本批次（tenant_id 注入）新增测试全部落在 pin 套件之外**（AC-1a `test/jwt_claims_test.dart` 单测 + AC-1b dashboard tenant-scoping widget 测试——均不在 pin 四文件/entry_ux/liveness/API 清单内），pin 计数不变；未来批次触碰 pin 内文件时须以 `flutter test … -r expanded` 实测重钉；**F8 计数断言已落地**（`Makefile` `guard-count-pin` 目标 + `ci.yml` 同门 step，实测 **OK +82**）——计数钉 **82**，与 G7 行同钉：删除任何 guard 测试即 +82 不匹配 → CI 失败；四文件任一变更须连同重测重钉 |
| G8（生产 cutover） | 全部批次 + 迁移骨架 P1–P3 | 各仓 CI 全绿、首个事件验证、DLQ/alert 上线（H2/H6） |

**要点**：① 批次间严格按 v2 rollout 顺序（sink → aero-id → aero-vault → IdP → aero-im → console），仅 B4-3/B4-4 与 B2/B3 可并行；② 所有验收断言均映射审查测试编号（T-1.1 至 T-12）与 drill（D1），可逐条回归；③ aero-vault B3-5 与 aero-im B5-2 的"scope 对齐/配给"依赖 B4-2（IdP scope registry），这是 F2 契约修订后的唯一剩余依赖链；④ G7(B6) 的 B6-1 计数断言必须在**同一次 CI 门禁**中同时运行 pin 套件（guard+widget +27，含 3 个已落地 developer 测试与 B6-1 developer 负边界 8 项）与 liveness 套件（+15）：`@TestOn('vm')` 在非 VM 平台静默跳过、仅 exit-code 无法拦截（M5），且 no-write pin 的对比只有在 liveness 同门证明 ring 存活时才有意义（M10）。
