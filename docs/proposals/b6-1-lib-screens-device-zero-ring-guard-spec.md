...
Status: implemented and verified (2026-08-20). The device-module zero-ring guard, no-write decision tests, and redirect-leg pins are green; the historical pipeline-failure note is retained in the body for provenance.
...
6. **Liveness contrast (anti-vacuous):** the no-write pin is only meaningful while the ring is provably live — ring liveness for 2xx non-GET mutations remains pinned by the liveness group in `test/snaplink_admin_api_test.dart` (19 tests including the F12 transport pins; `AuditLogService` is a singleton, so the same `count` the liveness test exercises is the one REQ-2 asserts stays 0).
