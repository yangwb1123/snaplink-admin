...
> Status: **design of record for the build request** `docs/proposals/b6-2-lib-screens-portal-session-expiry-rollback-pin-spec.md`, verified against HEAD `de9b446` + working tree on 2026-08-08. Every claim in the supplied spec was re-checked against the live tree and every acceptance command was **executed** rather than trusted. One material discrepancy found (AC-5's `portal_entry_test` leg is a silent VM skip — see §0 row 12 and §5); all other divergences are off-by-one citation drift inside the spec's own `[CORRECTION]` markers, substance identical. Sibling spec: `docs/proposals/b6-2-lib-screens-portal-session-expiry-rollback-pin-spec.md`. Precedent format: `docs/proposals/b6-2-lib-screens-portal-edge-generation-design.verified.md`.
...
| Pin-neutrality: `portal_api_test.dart`/`portal_entry_test.dart` in no count-gated suite; `portal_api.dart` is a 3-line re-export shim | `grep -rl portal_api_test\|portal_entry_test test/ Makefile ci.yml` → **zero hits**; G7 count row (`implementation-gate.md:79`) names only guard 4-file suite (82), `entry_ux_test` (10), `snaplink_admin_api_test` (15), `device_verify_api_test` (4); shim = 196-byte `export` (read) | ✅ exact |
...
**Bottom line:** every substantive claim — the four gaps (S1-S4), every hook call site, every rollback span, the premise that the five existing tests never arm the hook — is **confirmed at HEAD**. The single material defect is AC-5's entry-test leg: as written it silently executes zero tests. The design below keeps the spec's REQ-1..REQ-4 and AC-1..AC-4 1:1 and **corrects AC-5** into a VM leg (the new suite), a browser leg (the request-bound guard, platform-explicit), and the zero-lib leg, with an optional one-line Makefile companion that closes the CI orphan.
...
4. **Makefile/CI untouched by the default contract**; the optional companion (constraint 5) touches only the `test-browser` target's file list — not `lib/`, not any count-gated file, not the guard-count-pin target (its `GUARD_PIN_FILES`/`GUARD_PIN_COUNT` are unaffected).
5. **Optional companion (recommended):** append `test/portal_entry_test.dart` to the `make test-browser` file list (Makefile `:85`). This is the minimal change that makes the "already-pinned login leg" actually execute in CI (it currently runs nowhere). Prerequisite verified: 13/13 green under chrome at HEAD — adding it cannot red CI. If deferred, it must be tracked as a follow-up; the AC-5 browser leg remains executable manually.
...
7. **No re-pinning needed anywhere:** G7 count (82), entry_ux (10), liveness (15), API (4) arithmetic is untouched — the appended file is uncounted and the browser target has no count assertion.
...
**Suite totals (this direction's scope):** VM `test/portal_api_test.dart` 42/42; browser `test/portal_entry_test.dart` 13/13; zero-lib gate empty. No count-gated suite is touched (G7 82 / entry_ux 10 / liveness 15 / API 4 all unchanged).
