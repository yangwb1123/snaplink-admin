...
> Every citation below was re-verified at HEAD (2026-08-07) before writing this design; precedent tests `test/oidc_login_screen_client_id_test.dart` + `test/oidc_login_handle_success_census_test.dart` were executed and are green (+9).
...
1. **Step 1 — now, at HEAD (zero `lib/` edits).** Land `test/oidc_login_audit_visibility_guard_test.dart` (§1.1), `test/oidc_login_ring_isolation_test.dart` (§1.2), and the ring-liveness pin in `test/snaplink_admin_api_test.dart` (§1.5). **Implemented and green at HEAD on 2026-08-07** — validation run: `flutter test test/oidc_login_audit_visibility_guard_test.dart test/oidc_login_ring_isolation_test.dart test/oidc_login_screen_client_id_test.dart test/oidc_login_handle_success_census_test.dart test/snaplink_admin_api_test.dart` → **+26 all passed** (step-1 11/11 + pin file 15/15); `flutter analyze` → no issues; both grep forms exit 1. The live run additionally surfaced D5 (census pin) and confirmed the D1 snapshot corrections and the D3 `kIsWeb` determinism. Green checks: the same four-file run; `grep -rn "AuditLogService\|audit_log_service\|sso_audit_log" lib/screens/oidc_login/` → exit 1; `flutter analyze` clean.
