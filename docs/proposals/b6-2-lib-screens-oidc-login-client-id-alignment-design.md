...
| F7 | **Vacuous green** — a co-site that "references the constant" by string interpolation or a renamed-away literal | Compile error (absent symbol) or census red (fresh literal — contiguous single/double-quoted form, or bare `sso-admin-` prefix in non-census test files) | Compile-time references only (D1). **Corrected to measured reality (security review G1–G4, hardening amendment):** the pre-amendment claim "a fresh literal is caught regardless of how it's produced" is retracted — G1 (double-quoted), G2 (split-half), G3 (escape suffix), G4 (interior split) all evaded the raw-text census in mutation (E3–E6). Amendment A3 closes G1–G3 at the gate (validated red); G4 is an accepted residual (amendment §R1) backed by the wire-level layers (harness `lastClientId`, drill `AGREED_CLIENT_ID`) |
| F8 | **Census self-hit / self-miss** — the census file scanning itself, or the derivation string drifting from the real declaration | Gate red in a way that looks like a false failure | Self-hit: handled (the literal is split — `"'sso-admin-" "console'"` — so the file never self-hits). **Detection: corrected to measured reality (security review G5 + WT rename-relaxation, hardening amendment):** the pre-amendment claims "a renamed constant flips the gate red" and "forcing a conscious update" were false at the WT gate — `constantExists`'s unanchored `contains()` and the decl-line substring check both match `firstPartyClientId2`, and a comment replica of the declaration satisfies both (G5 spoof: gate advertised zero-tolerance with no constant present). Amendment A2 restores the property with a line-anchored non-comment `\b`-bounded declaration match — rename → absent-branch red, comment spoof → absent-branch red (both re-proven by mutation) |
...
**Adversarial hardening (security review G1–G7, post-design):** F7/F8's textual-detection claims above are corrected to measured reality, and the gate is hardened against the reviewer's proven evasion forms (double-quoted/split/escape literals, flip-detector comment spoof, guard self-silencing) by `docs/proposals/b6-2-lib-screens-oidc-login-m2-census-hardening-amendment.md` — amendments A1–A4, accepted residuals R1/R2, and the P1–P13 pass matrix. Count pins (10/3/17) and the 43-gate joint are untouched by the amendments (verified 43/43).
