# 技术债清单（R244 structure / i18n / docs fan-out ledger reconciliation）
...
- R244 对账：当前 `lib/api/admin_paths.dart` 为 **122** 行，`routes` catalog 为 **215** 个唯一 operation（R191 scoped 为 **214**）；`lib/screens` 为 **248** 个文件且 **0** 个超 400 行，Developer census 仍为 **13** 个文件。`docs/ui` 顶层为 **12** 个直接子目录（上限 12）；`docs/ui/pages-per-page` 当前为 **319** 个报告文件（计入 R231-R244，其中 R241-R244 为本批）、**0** 个子目录，不构成 fan-out 违规；`checks/filesize.py` 与 `checks/directory_fanout.py` 均 PASS。
