...
## P2 结构债（advance 扫描，61 项）

| 类别 | 数量 | 说明 |
|---|---|---|
| 上帝文件 >400 行 | 37 | 当前工作树中的 37 个超预算 Dart 文件均已在 `engineering.yaml` 显式登记：i18n/API 数据或客户端、复杂状态页与本轮安全/治理功能页；未提高全局 400 行上限，新文件仍会触发门禁。dashboard 的低频导航已拆到 `dashboard_navigation_tail.dart`，登录、SCIM Bulk、分布式集群、租户品牌、审计日志、通知中心与组织列表面板的视图组合已分别拆到独立 part，其余页面按功能迭代继续组件化 |
