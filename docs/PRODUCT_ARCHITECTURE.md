...
| 设备码用户 | 看清请求方和权限后批准或拒绝设备登录 | `/device/verify` |
...
- 可信浏览器 grant 才允许按 client ID 存入 `localStorage`，并可由用户撤销。
...
3. 完成密码或外部身份源登录；challenge 失效后重新认证。
4. Consent 显示服务端权威 Scope/RAR，再同意或拒绝。
