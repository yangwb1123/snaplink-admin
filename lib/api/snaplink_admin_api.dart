...
/// 平台审计事件（snaplink audit 账本时间线）。
/// GET /api/v1/audit/events?limit=&type= — admin:read 门禁。
class PlatformAuditEvent {
  ...
  factory PlatformAuditEvent.fromJson(Map<String, dynamic> json) {
    ...
    return PlatformAuditEvent(
      ...
      actorIp: json['actor_ip']?.toString() ?? '',
...
extension SnaplinkAdminApiAudit on SnaplinkAdminApi {
  /// 拉取平台审计时间线（snaplink 账本，最新在前）。
  Future<List<PlatformAuditEvent>> fetchPlatformAuditEvents({
    String? type,
    int limit = 200,
  }) async {
    final data = await get('/api/v1/audit/events', query: {
      'limit': '$limit',
      if (type != null && type.isNotEmpty) 'type': type,
    });
    final events = (data['events'] as List<dynamic>? ?? [])
        .map(
          (e) => PlatformAuditEvent.fromJson(
