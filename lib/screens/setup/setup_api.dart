...
class SetupStatus {
  ...
  const SetupStatus(this.setupRequired);
}
...
class SetupApplication {
  ...
  Map<String, dynamic> toJson() => {
        'name': name,
        if (redirectUri != null && redirectUri!.isNotEmpty) 'redirect_uris': [redirectUri],
      };
}
...
class SetupResult {
  ...
  const SetupResult.success({required String? admin, String? clientId, String? clientSecret})
      : this._(createdAdmin: admin, clientId: clientId, clientSecret: clientSecret);
...
class SetupApi {
  ...
  /// Mirrors app.js's IIFE `init()`: a non-OK response, an unparsable body,
  /// or a thrown network exception ALL fall open into the wizard (matching
  /// the JS default `{setup_required: true}` / catch -> showView('admin')).
  Future<SetupStatus> checkStatus() async {
    try {
      final resp = await _http.get(_statusUri(), headers: const {'Accept': 'application/json'});
      if (resp.statusCode < 200 || resp.statusCode >= 300) {
        return const SetupStatus(true);
      }
      final decoded = jsonDecode(resp.body);
      if (decoded is Map<String, dynamic> && decoded['setup_required'] == true) {
        return const SetupStatus(true);
      }
  ...
  Future<SetupResult> submit({required SetupAdmin admin, SetupApplication? application}) async {
    ...
    return SetupResult.failed(err != null ? 'Setup failed: $err' : 'Setup failed.');
  }
}
