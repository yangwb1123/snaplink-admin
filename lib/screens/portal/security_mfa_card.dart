  ...
  Widget build(BuildContext context) => PortalCard(
    ...
    children: [
      if (mfaLoading)
        ...
        for (final raw in factors) SecurityFactorTile(factor: raw as Map, busy: mfaBusy, onRemove: (id) => onRemoveFactor(id)),
      MessageBanner(mfaMessage, ok: mfaMessage == 'Second factor removed.'),
      const SizedBox(height: 8),
      OutlinedButton(onPressed: mfaBusy ? null : onBeginTotp, child: const Text('Add authenticator app')),
      if (totpPanelOpen) ...[
        ...
        if (pendingUri.isNotEmpty) SelectableText(pendingUri, style: const TextStyle(color: Color(0xFF6366F1))),
        const SizedBox(height: 10),
        TextField(controller: totpLabelCtrl, decoration: const InputDecoration(labelText: 'Device name (optional)')),
        const SizedBox(height: 10),
        TextField(controller: totpCodeCtrl, decoration: const InputDecoration(labelText: '6-digit code'), keyboardType: TextInputType.number),
        const SizedBox(height: 10),
        Align(
