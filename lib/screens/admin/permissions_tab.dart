...
class _PermissionsTabState extends State<PermissionsTab> {
  ...
  Future<void> _editRole([Map<String, dynamic>? existing]) async {
    ...
    if (!await _confirm(
      ...
      '${editing ? "Update" : "Add"} role ${role.code} for $cid?',
    )) {
      return;
    }
    await _write(
  ...
  Future<void> _deleteRole(String code) async {
    ...
    if (!await _confirm(
      'Delete role?',
      'Delete $code from $cid? This cannot be undone.',
      destructive: true,
      confirmText: code,
    )) {
      return;
  ...
  Future<void> _unassign(String uid) async {
    ...
    if (!await _confirm(
      'Unassign user?',
      'Remove all role assignments for $uid?',
      destructive: true,
      confirmText: uid,
    )) {
      return;
  ...
  Future<void> _write(Future<void> Function() fn, String okMsg) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: LocalizedText(okMsg)));
      await _load();
    } on SnaplinkAdminApiError catch (e) {
      if (mounted) setState(() => _error = e.toString());
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        if (clientId != null)
          PermissionSectionSelector(
            selectedSection: _currentSection,
            onSelected: _selectSection,
          ),
        if (clientId != null &&
            (_currentSection == 'all' || _currentSection == 'roles') &&
            _supportsRoles) ...[
          const SizedBox(height: 12),
          _roleSection(context),
        ],
        if (clientId != null &&
            (_currentSection == 'all' || _currentSection == 'assignments') &&
            _supportsAssignments) ...[
          const SizedBox(height: 12),
          _assignmentSection(context),
        ],
        if (clientId != null &&
            (_currentSection == 'all' || _currentSection == 'menus') &&
            _supportsMenus) ...[
          const SizedBox(height: 12),
          PermissionMenusCard(
